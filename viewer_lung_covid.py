...
class Main(QMainWindow, Ui_MainWindow):
    def __init__(self, ):
        ...
        self.ResetAll_Button.clicked.connect(self.ResetAll_ContourAndDeleteJsonFile)
        self.Show_Button.clicked.connect(self.Reload_Json_File2)
        self.Hidden_Button.clicked.connect(self.Hidden_Contour)
    ...
    def ResetAll_Contour(self,fig):
        ...
        json_file = self.NomDuFichierLabel.text()
        json_file.replace('.dcm','.json')
        # os.unlink(json _file)
    ...
    def ResetAll_ContourAndDeleteJsonFile(self,fig):
        self.canvas.figure.gca().clear()
        dataset = pydicom.dcmread(self.NomDuFichierLabel.text())
        self.Reload_Json_File(self)
        dicom_file = self.NomDuFichierLabel.text()
        json_file = dicom_file.replace('.dcm','.json')
        json_file = json_file.replace('/dataset/POSITIFS/', '/json_files/')
    ...
    def Reload_Json_File2(self,fig):
        ...
        if os.path.exists(JSON_FILE):  # lecture du log file de l'image en cours
            ...
            k = 1
            p = 0  # curseur (point de depart de la forme suivante)
            while k < (len(self.select_o)-5):
                if ((k == 1) or (p == 1)):
                    x0 = self.select_o[k]
                    y0 = self.select_o[k + 1]
                    p = 0
                x1 = self.select_o[k]
                y1 = self.select_o[k + 1]
                x2 = self.select_o[k + 2]
                y2 = self.select_o[k + 3]
                k = k + 2
            ...
            k = 1
            p = 0  # curseur (point de depart de la forme suivante)
            while k < (len(self.select_y)-5):
                if ((k == 1) or (p == 1)):
                    x0 = self.select_y[k]
                    y0 = self.select_y[k + 1]
                    p = 0
                x1 = self.select_y[k]
                y1 = self.select_y[k + 1]
                x2 = self.select_y[k + 2]
                y2 = self.select_y[k + 3]
                k = k + 2
                self.line_color = 'yellow'
                self.canvas.figure.gca().add_artist(patches.mlines.Line2D([x1, x2], [y1, y2], color=self.line_color, linestyle='solid', linewidth=0.1))
                if self.select_y[k+4] == 0:
                    self.line_color = 'yellow'
                    self.canvas.figure.gca().add_artist(
                    patches.mlines.Line2D([x2, x0], [y2, y0], color=self.line_color, linestyle='solid', linewidth=0.1))
                    k = k + 5
            ...
            while k < (len(self.select_r) - 5):
                ...
                self.line_color = 'red'
                self.canvas.figure.gca().add_artist(
                    patches.mlines.Line2D([x1, x2], [y1, y2], color=self.line_color, linestyle='solid', linewidth=0.1))
                if self.select_r[k + 4] == 0:
                    self.line_color = 'red'
                    self.canvas.figure.gca().add_artist(
                        patches.mlines.Line2D([x2, x0], [y2, y0], color=self.line_color, linestyle='solid',
                                              linewidth=0.1))
                    k = k + 5
                    p = 1
